...
const Home = ({ clothesdetail }) => {
  ...
  return (
    <div className="relative min-h-full overflow-hidden pb-28 md:pb-4">
      ...
      <div className="relative space-y-5 sm:space-y-6">
        ...
        <section className={`${glass} rounded-[2rem] overflow-hidden`}>
          ...
          <div className="fixed left-3 right-3 bottom-3 z-40 md:static md:border-t md:border-white/70 md:bg-white/20"><div className="mx-auto max-w-xl rounded-2xl border border-white/80 bg-white/65 backdrop-blur-xl px-3 py-3 shadow-[0_12px_40px_rgba(15,23,42,0.16)] md:max-w-none md:rounded-none md:border-0 md:bg-transparent md:px-5 md:py-5 md:shadow-none"><div className="flex items-center gap-3"><div className="min-w-0 flex-1"><p className="text-xs font-medium text-slate-500">Wash date</p><input type="date" value={washDate} onChange={e => setWashDate(e.target.value)} className="mt-1 w-full min-w-0 border-0 bg-transparent p-0 text-sm font-semibold text-slate-800 outline-none focus:ring-0" /></div><button onClick={handleAddToWash} disabled={!authUser || !selectedClothes.length} className="shrink-0 rounded-xl bg-slate-900/90 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-45">🧺 Add</button></div></div></div>
        </section>
      </div>
